 ...
 * <p/>
 * Non-primitive objects are processed recursively so that a string representation of inner objects
 * is also generated. Too avoid too much output, this recursion is limited with a given maximum
 * depth.
 *
 * @author Tim Ducheyne
 * @author Filip Neven
...
public class ObjectFormatter {
  ...
  private static final String MOCK_NAME_CHAIN_SEPARATOR = "##chained##";
   ...
   * <p/>
   * NOTE: there is no cycle detection. A large max depth value can cause lots of output in case of
   * a cycle.
   *
   * @param maxDepth The max depth > 0
   * @param maxNrArrayOrCollectionElements The maximum nr of elements for arrays and collections to
   * display  > 0
   */
  public ObjectFormatter(int maxDepth, int maxNrArrayOrCollectionElements) {
    ...
    this.arrayAndCollectionFormatter = new ArrayAndCollectionFormatter(
  ...
  protected boolean formatMock(Object object, StringBuilder result) {
    try {
      ...
      String mockName = (String) proxyUtilsClass.getMethod("getMockName", Object.class)
          .invoke(null, object);
      if (mockName == null) {
        return false;
      }
  ...
  protected boolean formatProxy(Object object, Class<?> type, StringBuilder result) {
    ...
    if (index > 0) {
      result.append("Proxy<");
      result.append(className, 0, index);
      result.append(">");
      return true;
    }
    return false;
  ...
  /**
   * @return The interface that represents a dummy object. If the DummyObject interface is not in
   * the classpath, null is returned.
   */
