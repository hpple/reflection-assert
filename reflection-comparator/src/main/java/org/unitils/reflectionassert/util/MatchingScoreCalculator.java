...
/**
 * A utility class to be able to calculate a score of how well 2 elements match. This enables us to
 * find the best matching differences out of all element differences in an unordered collection
 * difference.
 *
 * @author Tim Ducheyne
 * @author Filip Neven
...
public final class MatchingScoreCalculator {
   ...
   */
  private final Map<Difference, Integer> cachedMatchingScores = new IdentityHashMap<Difference, Integer>();
  ...
  /**
   * Gets the matching score for a simple difference. This will return 0 in case both objects are of
   * the same type. If both objects are of a different type, they are less likely to be a best
   * match, so 5 is returned.
   *
   * @param difference The difference, not null
   * @return The score
   */
  private int getMatchingScore(Difference difference) {
    Object leftValue = difference.getLeftValue();
    Object rightValue = difference.getRightValue();
    if (leftValue != null && rightValue != null && !leftValue.getClass()
        .equals(rightValue.getClass())) {
      return 5;
    }
    return 1;
  }
   ...
   * @return The score
   */
  private int getMatchingScore(ObjectDifference objectDifference) {
   ...
   * @return The score
   */
  private int getMatchingScore(MapDifference mapDifference) {
   ...
   * @return The score
   */
  private int getMatchingScore(CollectionDifference collectionDifference) {
    return collectionDifference.getElementDifferences().size();
  }


  /**
   * Gets the matching score for an unordered collection difference. Returns the sum of the matching
   * scores of the best matches.
   *
   * @param unorderedCollectionDifference The difference, not null
   * @return The score
   */
  private int getMatchingScore(UnorderedCollectionDifference unorderedCollectionDifference) {
