 ...
 * <p/>
 * Every operation is performed through reflection to avoid a direct link to Hibernate. This way you
 * do not need Hibernate in the classpath to use the reflection comparator.
 *
 * @author Tim Ducheyne
 * @author Filip Neven
 */
public final class HibernateUtil {
   ...
   * <p/>
   * If the given object is not a proxy or if Hibernate is not found in the classpath, this method
   * just returns the given object. If the given object is a proxy, the proxy is initialized
   * (loaded) and the un-wrapped object is returned.
   *
   * @param object The object or proxy
   ...
   */
  private static Object invokeLazyInitializerMethod(String methodName, Object proxy) {
    try {
      Object lazyInitializer = hibernateProxyClass.getMethod("getHibernateLazyInitializer")
          .invoke(proxy);
      return lazyInitializer.getClass().getMethod(methodName).invoke(lazyInitializer);
    } catch (Exception e) {
      throw new UnitilsException(
          "Unable to invoke method on lazy initializer of Hibernate proxy. Method: " + methodName
              + ", proxy: " + proxy,
          e
      );
    }
