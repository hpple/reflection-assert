...
/**
 * Formatter that will output all leaf differences in the tree and, in case of an unordered
 * collection difference, the difference of all best matches.
 *
 * @author Tim Ducheyne
 * @author Filip Neven
 */
public class DefaultDifferenceView implements DifferenceView {
  ...
  protected String formatDifference(ObjectDifference objectDifference, String fieldName) {
    ...
    for (Map.Entry<String, Difference> fieldDifference : objectDifference.getFieldDifferences()
        .entrySet()) {
  ...
  protected String formatDifference(ClassDifference classDifference, String fieldName) {
    StringBuilder result = new StringBuilder();
    result.append("Expected: object of type ")
        .append(classDifference.getLeftClass().getSimpleName());
    result.append(", actual: object of type ")
        .append(classDifference.getRightClass().getSimpleName()).append("\n");
    return result.toString();
  }
  ...
  protected String formatDifference(CollectionDifference collectionDifference, String fieldName) {
    ...
    for (Map.Entry<Integer, Difference> elementDifferences : collectionDifference
      ...
      result
          .append(elementDifferences.getValue().accept(differenceFormatterVisitor, innerFieldName));
    }
  ...
  protected String formatDifference(
    ...
    if (unorderedCollectionDifference.getRightList().size() != unorderedCollectionDifference
        .getLeftList().size()) {
      result.append("Collections have a different size: Expected ")
          .append(unorderedCollectionDifference.getLeftList().size()).append(", actual ")
          .append(unorderedCollectionDifference.getRightList().size()).append(".\n");
    }
    ...
    for (Map.Entry<Integer, Integer> bestMatchingIndex : bestMatchingIndexes.entrySet()) {
      ...
      if (leftIndex == -1) {
        ...
        result.append(formatValues(innerFieldName,
            NO_MATCH,
  ...
  protected String formatValues(String fieldName, Object leftValue, Object rightValue) {
    ...
    String valuesFormattedOnOneLine = formatValuesOnOneLine(
        fieldName,
        leftValueFormatted,
        rightValueFormatted
    );
  ...
  protected String formatValuesOnOneLine(
      String fieldName,
  ...
  protected String formatValuesOnMultipleLines(
      String fieldName,
      String leftValueFormatted,
      String rightValueFormatted
  ) {
