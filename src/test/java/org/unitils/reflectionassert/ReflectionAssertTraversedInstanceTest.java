 ...
 * <p/>
 * Special thanks to Dmitry Sidorenko, who first reported this issue and also indicated how to solve
 * it.
 *
 ...
 * @author Filip Neven
 */
class ReflectionAssertTraversedInstanceTest {


  /**
   * Test comparing objects with two lists which have intersecting list elements
   * <p/>
   * When traversing first collection child1 and child2 are marked as already compared. Child1 and
   * child2 were found not equal, but the reflection assert class only stores the fact that they
   * were already traversed, not the outcome (not equal). When traversing second collection, child1
   * and child2 are compared again but because they were already once compared, they are found
   * equal. Which is ofcourse incorrect.
   * <p/>
   * The reflection assert class has been changed to also remember the outcome of traversed instance
   * pairs.
   */
  @Test
  void doubleCheckTest() {
    ...
    assertLenientEquals(root1, root2);
  }
   ...
   * This test case is highly dependent on order of fields in collection returned by {@link
   * Class#getDeclaredFields()}
   */
  @Test
  void doubleCheckTestShouldFail() {
    Parent root1 = new Parent("root");
    Parent root2 = new Parent("root");
